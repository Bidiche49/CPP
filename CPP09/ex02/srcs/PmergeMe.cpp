...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/24 19:22:26 by ntardy            #+#    #+#             */
/*   Updated: 2024/03/25 01:35:32 by ntardy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PmergeMe::fillCont(std::string const & entry) {
	...
	for (size_t i = 0; i < entry.size(); i++) {
		if (i != 0 && isdigit(entry[i - 1]) && entry[i] == ' ') {
			std::istringstream iss(entry.substr(start, i - start));
	...
	std::cout << "Time to process a range of " << _nbEntry << " elements with" << BLUE << " std::vector : " << _vtime << " us" << RESET << std::endl;
	std::cout << "Time to process a range of " << _nbEntry << " elements with" << BLUE << " std::deque : " << _dtime << " us" << RESET << std::endl;
}
...
void PmergeMe::printResV(std::vector<int> vPrint) {
	...
	std::cout << "After with" << BLUE << " std::vector : " << tmp << RESET << std::endl;
}
...
void PmergeMe::printResD(std::deque<int> vPrint) {
	...
	std::cout << "After with" << BLUE << " std::deque : " << tmp << RESET << std::endl;
}
